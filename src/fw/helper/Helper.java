...
import annotation.controller.MyController;
import annotation.param.MyRequestParam;
import annotation.url.GetUrl;
import annotation.url.MyUrl;
import annotation.url.PostUrl;
import util.CMethod;
...
public class Helper {
    ...
    private Object convertParameter(Parameter parameter, Method method, HttpServletRequest request) throws Exception {
        ...
        if (parameter.isAnnotationPresent(fw.annotation.session.SessionInjection.class)) {
            annotation.session.SessionInjection sessionAnnotation = parameter.getAnnotation(fw.annotation.session.SessionInjection.class);
            String attributeKey = sessionAnnotation.value();
            
            session.Session session = (fw.session.Session) request.getAttribute("session");
            if (session == null) {
                return null;
            }
